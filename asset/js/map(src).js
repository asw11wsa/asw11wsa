...
var cart_load_pos  = function(locations,obj,cid) {
		...
		success: function(JSONObject) {
				...
				if (JSONObject.hasOwnProperty(key)) {
						...
						if(fn_pre_item_check(JSONObject[key]["idx"])==null){
                            ...
                            PlanCartHtml  += '<tr>';
							PlanCartHtml  += '<td class="redips-mark" style="height:0px;">';
							PlanCartHtml  += '<div class="redips-drag  cart cart-redips-clone " style="width:280px;height:65px;border:0px;background: #fcfaf5;font-size:14px;"'; 
							PlanCartHtml  += ' id="'+JSONObject[key]["idx"]+'@'+ JSONObject[key]["r_lat"]+ '@' + JSONObject[key]["r_lng"]+'@"  style="border:0px">';
							PlanCartHtml  += '									  <table  class="redips-nolayout">';
							PlanCartHtml  += '											<tr>';
							PlanCartHtml  += '												<td class="img">';
							PlanCartHtml  += '													<img src="/manager/restaurant/scfile/thumnail/' + JSONObject[key]["img"] + '" alt="" id="fimg">';
							...
							PlanCartHtml  += '												</td>';
							PlanCartHtml  += '												<td class="tit">';
							PlanCartHtml  += '													<div class="srctitle" style="display:none">' + JSONObject[key]["s_name"] + '</div> ';
							PlanCartHtml  += '													<p class="s_position" style="display:none">1,01</p>';
							PlanCartHtml  += '													<span class="r_lat" style="display:none">' + JSONObject[key]["r_lat"] + '</span>';
							PlanCartHtml  += '													<span class="r_lng" style="display:none">' + JSONObject[key]["r_lng"] + '</span>';
							PlanCartHtml  += '													<p class="category" style="display:none">' + JSONObject[key]["cate"] + '</p>';
							PlanCartHtml  += '													<p class="itemid" style="display:none">' + JSONObject[key]["idx"] + '</p>';
							PlanCartHtml  += '													<p class="city" style="display:none">' + JSONObject[key]["city"] + '</p>';
							PlanCartHtml  += '													<span class="cart_item_title">'+ JSONObject[key]["s_name"] + '</span>';
							PlanCartHtml  += '												</td>';
							PlanCartHtml  += '												<td class="utill">';
							PlanCartHtml  += '													<span class="rlabel">'+num_call_max_auto+'</span>';
				...
				*/
					...
					//해당마크삭제후 넘버링번호 마크를 찍어준다. 
					 ...
					 var markerIcon = {
					  ...
					  var marker = new google.maps.Marker({
						...
						infow[infow.length]=markinfo_pop;
					 ...
					 /*
					 for(var i=0, l=marks.length;i < l;i++){
                            console.log(idx + ":" + marks[i].get("id"));
							if(idx==marks[i].get("id")){
								marks[i].setMap(null);
							    matchi = i;
							}
					 }
					 ...
					 console.clear();
					 for(var i=0, l=marks2.length;i < l;i++){
                            console.log(idx + ":" + marks2[i].get("id"));
							if(idx==marks2[i].get("id")){
								marks2[i].setMap(null);
								matchi = i;
								break;
							}
					 }
					 marks[arr_index]=marks2[i];
					 
					 
                   
				  // one_add_mark(idx,item_name,icon,lat,lng,num_call_max_auto,first_img);
				  //add_mark_idx(idx,item_name,icon,lat,lng,num_call_max_auto,first_img);
				...
				try{
					//common_cart_point_load($("#country_txt").text());
				    redips.init();
					//setTimeout(function() { $("#cart_loader").hide();}, 1000); 
				}catch(e){}
 ...
 var  mnstourcart_add_idx = function(idx){
	$.ajax({
		async: false,
		type: "POST",
		data: {
